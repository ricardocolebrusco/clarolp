document.addEventListener("DOMContentLoaded", function () {
  ...
  function changePlanCard() {
    $(
      'input[name="netflix-bundle-soundbox"], input[name="netflix-bundle-box"]'
      ...
      const priceElement = $(this)
        .closest(".product-plan")
        .find("h5[data-price]")[0];
  ...
  function showPopUp() {
    $(".btn-compare").click(() => showExitIntentPopup("boxComparePlans"));

    $(".btn-compare-sound").click(() =>
      showExitIntentPopup("soundboxComparePlans")
    );
  }
  ...
  const radiosBox = document.querySelectorAll(
    'input[type="radio"][name="netflix-bundle-box-popup"]'
  );
  const radiosSoundbox = document.querySelectorAll(
    'input[type="radio"][name="netflix-bundle-soundbox-popup"]'
  );

  function handleRadioChange(event) {
    // Determine which group of radios triggered the change event
    const radios =
      event.target.name === "netflix-bundle-box-popup"
        ? radiosBox
        : radiosSoundbox;
  ...
  $(".box-claro-carrossel .owl-carousel").owlCarousel({
    margin: 0,
    loop: false,
    autoWidth: true,
    dots: false,
  ...
  function RedentionShowExitIntentPopup() {
    var popup = document.getElementById("exitIntentPopup");
    var background = document.getElementById("popupBackgroundRedention");

    var syntheticEvent = {
      currentTarget: popup,
    };

    pushGA4(syntheticEvent);
    ...
    if (!popup.classList.contains("active")) {
      ...
      document.body.style.overflow = "hidden"; // Desabilita o scroll do body
      sessionStorage.setItem("RedentionPopupCounter", ++RedentionPopupCounter);
    }
  }

  function RedentionCloseExitIntentPopup(e) {
    var popup = document.getElementById("exitIntentPopup");
    var background = document.getElementById("popupBackgroundRedention");
  ...
  function RedentionIsMobileDevice() {
    return (
  ...
  // Exibição e Retenção da Modal
  var RedentionPopupCounter =
    sessionStorage.getItem("RedentionPopupCounter") || 0;

  if (RedentionPopupCounter < 1) {
    if (!RedentionIsMobileDevice()) {
      document.addEventListener("mouseleave", function (e) {
        if (e.clientY < 0 && RedentionPopupCounter == 0) {
          RedentionShowExitIntentPopup();
          sessionStorage.setItem(
            "RedentionPopupCounter",
            ++RedentionPopupCounter
          );
        }
      });
  ...
  document
    .getElementById("closeBtnRedention")
    .addEventListener("click", RedentionCloseExitIntentPopup);
  ...
  document
    .getElementById("popupBackgroundRedention")
    .addEventListener("click", RedentionCloseExitIntentPopup);
});
